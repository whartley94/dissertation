...
base_call = f"python train.py --name Auto --sample_p 1.0 --niter 1 --niter_decay 0 --classification --display_id -1 " \
    f"--data_dir {sorted_path} --checkpoints_dir {cpoint_path} --phase train_small"

base_call2 = f"python train.py --name Auto --sample_p .125 --niter 1 --niter_decay 0 --lr 0.000001 --display_id -1 " \
    f"--data_dir {sorted_path} --checkpoints_dir {cpoint_path} --phase train --load_sg_model"

# train.py --name siggraph_reg2 --sample_p .125 --niter 5 --niter_decay 0 --lr 0.000001 --load_model --phase train
